...
class Anafapi:
    """Class to access ANAF API services"""
    ...
    def __init__(self, cui: str) -> None:
        """Class requires CUI (Romanian tax code) to instantiate the class"""
        self.classname: str = self.__class__.__name__
        self.cui: str = cui
        # Payload must be a list containing at least one dictionary tax code and current date, more codes can be queried at the same time
        # Documentation: https://static.anaf.ro/static/10/Anaf/Informatii_R/Servicii_web/doc_WS_V8.txt
        self.payload: list[dict] = [{"cui": self.cui, "data": self.__class__.today}]

    async def syncpost(self) -> httpx.Response | dict:
        """Asynchronous post requests to ANAF API services"""
        try:
